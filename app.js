...
const state = {
  ...
  locationMessage: "Standort noch nicht angefragt.",
...
const DISTANCE_SOURCE_LABELS = {
  geo_haversine: "Geo (Luftlinie/Haversine)",
  manual: "Manuell hinterlegt",
  legacy_manual: "Manuell (Legacy)",
  missing: "Nicht verfügbar",
};

const DISTANCE_QUALITY_LABELS = {
  high: "hoch",
  fallback: "Fallback",
  unavailable: "nicht verfügbar",
};
...
const locationStateIndicator = document.querySelector("#location-state");
...
locationForm.addEventListener("submit", (event) => {
  ...
  if (lat === null) {
    setLocationState("unknown", "Kein Standort gesetzt (manuelle Distanz-Fallbacks aktiv).");
  } else {
    setLocationState("granted", "Standort manuell gesetzt.");
  }
...
detectLocationButton.addEventListener("click", () => {
  if (!navigator.geolocation) {
    setLocationState("unavailable", "Geolocation wird vom Browser nicht unterstützt.");
    return;
  }

  setLocationState("requesting", "Standort wird abgefragt …");

  navigator.geolocation.getCurrentPosition(
    (position) => {
      const lat = position.coords.latitude;
      const lng = position.coords.longitude;
      state.settings.currentLocation = { lat, lng };
      persist(STORAGE_KEYS.settings, state.settings);
      setLocationState(
        "granted",
        `Standort ermittelt (±${Math.round(position.coords.accuracy)} m Genauigkeit).`
      );
      locationFeedback.textContent = "Standort automatisch gespeichert.";
      locationFeedback.className = "feedback good";
      render();
    },
    (error) => {
      if (error.code === error.PERMISSION_DENIED) {
        setLocationState("denied", "Standortzugriff wurde verweigert.");
      } else if (error.code === error.POSITION_UNAVAILABLE) {
        setLocationState("unavailable", "Standort ist derzeit nicht verfügbar.");
      } else {
        setLocationState("error", "Standortabfrage fehlgeschlagen (Timeout/technischer Fehler).");
      }
    },
    {
...
calcForm.addEventListener("submit", (event) => {
  ...
  const stationsWithEnergyPrice = state.stations
    .map((station) => {
      ...
      const distanceInfo = resolveStationDistanceInfo(station, state.settings.currentLocation);
      return Number.isFinite(price) && Number.isFinite(distanceInfo.km)
        ? { ...station, price, rawPrice, distance: distanceInfo.km, distanceInfo, exchangeRateUsed }
        : null;
    })
    .filter(Boolean);
  ...
  result.innerHTML =
    ...
    rows
      ...
      <div class="result-item">
        ...
        Distanzquelle: ${formatDistanceSource(station.distanceInfo.source)} · Qualität: ${formatDistanceQuality(station.distanceInfo.quality)} (${station.distanceInfo.reason})<br />
...
function setLocationState(stateKey, message) {
  state.locationState = stateKey;
  state.locationMessage = message;

  const classes = {
    granted: "feedback good",
    requesting: "feedback",
    unknown: "feedback",
    denied: "feedback bad",
    unavailable: "feedback bad",
    error: "feedback bad",
  };

  const label = LOCATION_STATE_LABELS[stateKey] || stateKey;
  locationStateIndicator.textContent = `Standortstatus: ${label} – ${message}`;
  locationStateIndicator.className = classes[stateKey] || "feedback";
}
...
function formatDistanceSource(source) {
  return DISTANCE_SOURCE_LABELS[source] || source;
}

function formatDistanceQuality(quality) {
  return DISTANCE_QUALITY_LABELS[quality] || quality;
}
...
function render() {
  ...
  detectLocationButton.disabled = state.locationState === "requesting";
  setLocationState(state.locationState, state.locationMessage);
  ...
  stationList.innerHTML = state.stations
    .map((station) => {
      const distanceInfo = resolveStationDistanceInfo(station, state.settings.currentLocation);
      const distanceLabel = Number.isFinite(distanceInfo.km)
        ? `${distanceInfo.km.toFixed(1)} km (${formatDistanceSource(distanceInfo.source)})`
        : "keine Distanz";
